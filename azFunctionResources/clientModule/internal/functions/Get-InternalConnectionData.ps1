...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'SecurityComplianceCenter.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'SecurityComplianceCenter.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'SecurityComplianceCenter.Client.ProtectedToken'
