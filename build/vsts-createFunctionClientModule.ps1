...
$parentModule = 'SecurityComplianceCenter'
if (-not $ModuleName) { $ModuleName = 'SecurityComplianceCenter.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\SecurityComplianceCenter'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\SecurityComplianceCenter.psd1"
$prereqHash = @{
